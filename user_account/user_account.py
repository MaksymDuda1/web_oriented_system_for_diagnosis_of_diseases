from flask import render_template,redirect,Blueprint,session,request,current_app,jsonify
from db.db import get_user,do_user_update_user,update_picture,get_email
...
def do_user_update():
    if request.method == 'POST':
        ...
        old_email = ''.join(str(email) for email in session['email'])
        checker = get_email(request)
        if checker and email != old_email:
            msg = {"msg": "Email already exists"}
            return jsonify(msg), 200
        else:
            result = do_user_update_user(request)
            msg = {"msg": "Record successfully updated"}
            return jsonify(msg), 200
